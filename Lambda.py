# This is pasted from what I entered on AWS Lambda. The screen recordings 
# of how Lex is working is also included in this Github file. 
...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        ...
        if risk_level is not None: 
            output_session_attributes['bonds_amount'] = risk_level_numbers(risk_level)["bonds_amount"]
            output_session_attributes['equities_amount'] = risk_level_numbers(risk_level)["equities_amount"]
    ...
    return close(
        ...
        "content": """Thank you for your information;
           we recommend your portfolio has {}% bonds (AGG) and {}$ equities (SPY).
            """.format(
            bonds_amount, equities_amount
            ),
        },
    )     


def risk_level_numbers(risk_level):
    ...
    if str.lower(risk_level) == "high":
        bonds_amount = 20
        equities_amount = 80
    return {'bonds_amount' : bonds_amount, 'equities_amount' : equities_amount}
...
def validate_data(age, investment_amount, intent_request): 
    ...
    if age is not None:
        if parse_int(age) < 0:
            return build_validation_result(False, "age", "That is not a valid age. Try again.")
        elif parse_int(age) >= 65:
            return build_validation_result(False, "age", "You need to be under 65 to qualify.")
    
    if investment_amount is not None:
        if parse_int(investment_amount) < 5000:
            return build_validation_result(False, "investment_amount", "The investment amount must be at least $5000.")
